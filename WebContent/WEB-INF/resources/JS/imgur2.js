(function (root, factory) {
    ...
    Imgur.prototype = {
        ...
        status: function (el) {
            var div = this.createEls('div', {className: 'status2'});

            this.insertAfter(el, div);
        },
        matchFiles: function (file, zone) {
            var status2 = zone.nextSibling;

            if (file.type.match(/image/) && file.type !== 'image/svg+xml') {
                document.body.classList.add('loading');
                status2.classList.remove('bg-success', 'bg-danger');
                status2.innerHTML = '';
                ...
                status2.classList.remove('bg-success');
                status2.classList.add('bg-danger');
                status2.innerHTML = 'Invalid archive';
            }
        },
