											...
											<c:forEach items="${Corders}" var="list" step="1"
												...
												<tr>
													<td class="highlight" align="center" nowrap="nowrap">
														<a href="<c:url value="/OrderList.controller"/>?orderId=${Corders[current.index].orderId}" value="${Corders[current.index].orderId}">${Corders[current.index].orderId.substring(3,6)}${Corders[current.index].orderId.substring(12,19)}</a>
													</td>
													...
													<c:choose>
														<c:when test="${ShippingNumToStr[Corders[current.index].shipping]=='QRcode電子票券'}">
															<td class="highlight" align="center" nowrap="nowrap"><a href="${attrAddress[current.index]}">QRcode載點</a></td>
														</c:when>
														<c:otherwise>
															<td class="highlight" align="center" nowrap="nowrap">
																<a href="<c:url value="/orderShippingDetail.controller"/>?orderId=${Corders[current.index].orderId}" value="${Corders[current.index].orderId}">配送詳情</a></td>
														</c:otherwise>
													</c:choose>
													...
													<c:choose>
														...
														<c:otherwise>
															<td class="highlight" align="center" nowrap="nowrap">
																<a href="<c:url value="/ShowChangeOrder.controller"/>?orderId=${Corders[current.index].orderId}" value="${Corders[current.index].orderId}">修改</a>
															</td>
														</c:otherwise>
													</c:choose>
													...
													<c:choose>
														<c:when test="${statusNumToStr[Corders[current.index].status]=='未付款'}">
															<td class="highlight" nowrap="nowrap">
																<a href="<c:url value="/CancelOrder.controller"/>?orderId=${Corders[current.index].orderId}&status=${Corders[current.index].status}" value="${Corders[current.index].orderId}">取消</a></td>
														</c:when>
														<c:otherwise>
