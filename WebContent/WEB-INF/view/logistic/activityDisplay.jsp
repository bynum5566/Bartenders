...
<body >
			...
			<c:forEach var="Activity" items="${activity}" varStatus="status">
				<div class="each" id="${Activity.activityId}">
					<fieldset style="width: 350px;border-radius:30px">
								...
								<c:when test="${Activity.targetNum-Activity.actualNum>0}">
									...
									<form align=right action="joinActivity.do" method="post"
										...
										<input type="hidden" name="activityId"
											value="${Activity.activityId}"> <select
											name="joinNum">
											<c:forEach begin="1"
						...
						<div id="outer">
							<div id="ground">
								<p id="limitP" class="num"><img src="images/arrowLimit.png"></p>
								<div id="targetD">
									<p id="targetP" class="num"><img src="images/arrowTarget.png"></p>
								</div>
								<div id="currentD">
									<img id="currentP" src="images/arrowCurrent.png">
								</div>
							</div>
						</div>
						...
						<span>
							...
							<button
								id="${Activity.status}${Activity.activityId}${Activity.userId}"
								class="close visible" style="display: none">結束活動</button> <input
							type="hidden" name="preUrl" value="${preUrl}">
						</span>
					...
					$(document).ready(function (){
						getMarkers(prefix,'${Activity.activityId}','${status.index}');
						});
	...
	$("#manage").on("click",function() {
		window.location.href = '<c:url value="/queryActivityByUser.do"/>?userId='+targetId;
		})
